...
export const Item = ({
  ...
  const onArchive = (event: React.MouseEvent<HTMLDivElement, MouseEvent>) => {
    ...
    if (!id) return;
    const promise = archive({ id }).then(() => router.push("/spaces"));
  ...
  const onCreate = (event: React.MouseEvent<HTMLDivElement, MouseEvent>) => {
    event.stopPropagation();
    if (!id) return;
    const promise = create({ title: "Untitled", parentSpace: id }).then(
      (spaceId) => {
        if (!expanded) {
          onExpand?.();
        }
  ...
  return (
    ...
    >
      ...
      {spaceIcon ? (
        <div className="mr-2 shrink-0 text-[18px]">{spaceIcon}</div>
      ) : (
